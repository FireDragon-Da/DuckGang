...
public class NewspaperController : MonoBehaviour
{
    ...
    public void UpdateQuote(float happiness, float hunger)
    {
        ...
        int quoteLevel = Mathf.Clamp(Mathf.CeilToInt(averageStat / 20f), 1, 5);

        string categoryName = $"Quote_Level_{quoteLevel}";
        string quoteStr = quacxiconSO.GetRandomLogFromCategory(categoryName);
    ...
    public void UpdateArticles(List<ArticleEvent> candidateArticles)
    {
        ...
        {
            SetTopArticle("Quiet Day", "Nothing interesting happened.");
            SetSecondArticle("Weather", "Still quacking sunny.");
            return;
        }
